...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-50 bg-white border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.07)]">
      ...
      <div className="mx-auto flex max-w-xl items-end justify-around px-0 pt-1 bottom-nav-ios" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 4px)" }}>
        {NAV_ITEMS.map((item) => {
          /* زر الإضافة المركزي */
          if (item.icon === null) {
            return (
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center gap-0.5 px-1 py-2 transition-opacity ${
